...
import gaia3d.domain.agent.DataLibraryConversionJobResult;
import gaia3d.domain.agent.DataLibraryConverterResultLog;
import gaia3d.domain.common.QueueMessage;
import gaia3d.domain.datalibrary.*;
import gaia3d.persistence.DataLibraryConverterMapper;
...
//            dataLibrary.setMappingType(mappingType);
            ...
            dataLibrary.setDataLibraryPath(dataLibraryPath);
            dataLibrary.setDataLibraryThumbnail(dataLibraryThumbnail);
            if(ruleId != null && ruleId.intValue() > 0) dataLibrary.setRuleId(ruleId);
            dataLibrary.setUserId(userId);
            //dataLibrary.setStatus(DataStatus.PROCESSING.name().toLowerCase());
            dataLibrary.setStatus(DataLibraryStatus.USE.name().toLowerCase());
