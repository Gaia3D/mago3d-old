...
public class DataLibraryUploadRestController {
	...
	public Map<String, Object> insert(MultipartHttpServletRequest request) throws Exception {
		...
		if( !StringUtils.isEmpty(request.getParameter("ruleId"))) dataLibraryUpload.setRuleId(Integer.valueOf(request.getParameter("ruleId")));
...
//		}
	 ...
	 * @return
	 */
	@PostMapping(value = "/{uploadDataLibraryId:[0-9]+}")
	public Map<String, Object> update(HttpServletRequest request, @PathVariable Long uploadDataLibraryId, @Valid DataLibraryUpload dataLibraryUpload, BindingResult bindingResult) {
		log.info("@@ dataLibraryUpload = {}", dataLibraryUpload);
		Map<String, Object> result = new HashMap<>();
		String errorCode = null;
		String message = null;
		...
		if(StringUtils.isEmpty(dataLibraryUpload.getDataLibraryName())) {
			errorCode = "data.library.name.empty";
		}
		if(StringUtils.isEmpty(dataLibraryUpload.getDataLibraryGroupId())) {
			errorCode = "data.library.group.id.empty";
		}
		if(StringUtils.isEmpty(dataLibraryUpload.getSharing())) {
			errorCode = "data.library.sharing.empty";
		}
		/*if(StringUtils.isEmpty(dataLibraryUpload.getDataType())) {
			errorCode = "data.library.type.empty";
		}*/
	 ...
	 * 선택 upload-data 삭제
	 * @param request
	 * @param dataLibraryUploadId
	 * @return
	 */
	@DeleteMapping(value = "/{dataLibraryUploadId:[0-9]+}")
	public Map<String, Object> deleteDatas(HttpServletRequest request, @PathVariable Long dataLibraryUploadId) {

		log.info("@@@@@@@ dataLibraryUploadId = {}", dataLibraryUploadId);
		Map<String, Object> result = new HashMap<>();
		String errorCode = null;
		String message = null;
		...
		dataLibraryUpload.setDataLibraryUploadId(dataLibraryUploadId);

		dataLibraryUploadService.deleteDataLibraryUpload(dataLibraryUpload);
