...
drop table if exists data_relation_info cascade;
...
comment on column data_group.ancestor is '조상';
comment on column data_group.parent is '부모';
...
comment on column data_info.data_relation_id is 'data 연관 관계 정보의 id';
...
comment on column data_info.assemble is '합체 가능한 데이터 유무. true : 합체, false : 단일';
...
create table data_relation_info (
    data_relation_id            bigint,
    relation_info               jsonb,
    constraint data_relation_info_pk 	primary key(data_relation_id)
);

comment on table data_relation_info is 'data 연관관계 정보';
comment on column data_relation_info.data_relation_id is '고유번호';
comment on column data_relation_info.relation_info is '데이터 조합 정보';
